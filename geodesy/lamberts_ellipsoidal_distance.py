...
def lamberts_ellipsoidal_distance(
    ...
    sigma = old_div(haversine_distance(lat1, lon1, lat2, lon2), EQUATORIAL_RADIUS)

    # Intermediate P and Q values
    P_value = old_div((b_lat1 + b_lat2), 2)
    Q_value = old_div((b_lat2 - b_lat1), 2)

    # Intermediate X value
    # X = (sigma - sin(sigma)) * sin^2Pcos^2Q / cos^2(sigma/2)
    X_numerator = (sin(P_value) ** 2) * (cos(Q_value) ** 2)
    X_demonimator = cos(old_div(sigma, 2)) ** 2
    X_value = (sigma - sin(sigma)) * (old_div(X_numerator, X_demonimator))

    # Intermediate Y value
    # Y = (sigma + sin(sigma)) * cos^2Psin^2Q / sin^2(sigma/2)
    Y_numerator = (cos(P_value) ** 2) * (sin(Q_value) ** 2)
    Y_denominator = sin(old_div(sigma, 2)) ** 2
    Y_value = (sigma + sin(sigma)) * (old_div(Y_numerator, Y_denominator))

    return EQUATORIAL_RADIUS * (sigma - ((old_div(flattening, 2)) * (X_value + Y_value)))
